...
int main(int argc, char* argv[])
{
	...
	asio::ip::address ip = asio::ip::address::from_string(address_, err);
	if (err.value() != 0)
	{
		printf("error\n");
	}

	asio::ip::tcp::endpoint ep(ip, port_number);

	//now we are ready
	asio::io_service ios;
	std::string host = "baidu.com";
	std::string port_num = "80";
	asio::ip::tcp::resolver::query query_(host, port_num, asio::ip::tcp::resolver::query::numeric_service);

	asio::ip::tcp::resolver resolver_(ios);
	asio::ip::tcp::resolver::iterator itEnd, it = resolver_.resolve(query_, err);
	if (err != 0)
	{
		std::cout << "error in resloving the " << host.c_str() << std::endl;
		std::cout << "message:" << err.message().c_str() << std::endl;
	}
	else
	{
		while (it != itEnd)
	...
	std::unique_ptr<int> newUp = std::move(up);
